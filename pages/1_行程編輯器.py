                ...
                with st.container(border=True):
                    ...
                    if st.button("➕ 加入行程", key=f"add_{place['place_id']}_{i}", use_container_width=True):
                        ...
                        new_item = {
                            ...
                            'transport_mode': 'driving'  # 👉 新增這一行：預設為開車
                        }
                        st.session_state.my_itinerary[st.session_state.current_day].append(new_item)
            ...
            try:
                ...
                for k in range(len(itinerary) - 1):
                    ...
                    res = gmaps.directions(
                        origin=f"{itinerary[k]['lat']},{itinerary[k]['lng']}",
                        destination=f"{itinerary[k+1]['lat']},{itinerary[k+1]['lng']}",
                        mode=mode,
                        language='zh-TW'
                    )
                    if res:
            ...
            for i, p in enumerate(iti):
                ...
                if i > 0:
                    ...
                    with st.popover(f"{mode_options.get(current_mode)}：{leg_info} ▾", use_container_width=True):
                        ...
                        # 👉 修正版：加入 idx 與 key_name 參數，打破 Closure 魔咒
                        def update_mode(idx, key_name):
                            new_mode = st.session_state[key_name]
                            st.session_state.my_itinerary[idx]['transport_mode'] = new_mode
                            auto_save_itinerary()
                        ...
                        st.divider()
                        nav_url = f"https://www.google.com/maps/dir/?api=1&origin={iti[i-1]['lat']},{iti[i-1]['lng']}&destination={p['lat']},{p['lng']}&travelmode={current_mode}"
                    ...
                    with st.spinner("AI 正在計算最佳路徑..."):
                        try:
                            ...
                            if directions_result:
                                ...
                                # 更新 Session State
                                st.session_state.my_itinerary[st.session_state.current_day] = new_iti
