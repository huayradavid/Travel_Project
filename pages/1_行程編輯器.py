        ...
        if not iti:
            ...
            for i, p in enumerate(iti):
                ...
                if i == 0:
                    ...
                    current_dt = datetime.datetime.combine(today, datetime.time(start_h, start_m))
                else:
                    # 後續站點：先計算「系統推算時間」= 前一站時間 + 停留時間 + 交通時間
                    prev_p = iti[i-1]
                    current_dt += datetime.timedelta(hours=prev_p.get('stay_h', 1), minutes=prev_p.get('stay_m', 0))
                    ...
                    if st.session_state.get('current_directions') and i-1 < len(st.session_state.current_directions):
                        ...
                        current_dt += datetime.timedelta(seconds=leg['duration']['value'])
                    
                    # 🛡️ 防延誤機制：如果使用者有「手動鎖定」這站的時間，就覆蓋系統推算！
                    if p.get('use_manual_arr', False):
                        current_dt = datetime.datetime.combine(today, datetime.time(p.get('arr_h', 8), p.get('arr_m', 0)))
                ...
                with st.container(border=True):
                    # 顯示抵達時間 (如果有手動更改，加上標記)
                    manual_badge = " <span style='font-size: 12px; color: #E74C3C; background: #FADBD8; padding: 2px 6px; border-radius: 4px;'>手動更改</span>" if p.get('use_manual_arr') else ""
                    
                    # 🚀 呼叫智能引擎，取得專屬標誌
                    place_icon = get_place_icon(p['名稱'])
                    
                    # 把寫死的 🚆 換成變數 {place_icon}
                    st.markdown(f"<div style='color: #1E3A8A; font-size: 18px; font-weight: 800; margin-bottom: 5px;'>{place_icon} {arrival_time_str} 抵達{manual_badge}</div>", unsafe_allow_html=True)
